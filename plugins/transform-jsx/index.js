...
module.exports =  function ({ types: t }) {
    ...
    const visitJSXElement = (path, state) => {
        ...
        const JSXElement = node => {
            const attrs = JSXAttributes(node.openingElement.attributes);
            console.log(attrs.properties);

            return jsxObjectTransformer(
                t.objectExpression([
                    t.objectProperty(t.identifier(nameProperty), JSXElementName(node.openingElement.name)),
                    t.objectProperty(t.identifier(attributesProperty), attrs),
                    t.objectProperty(t.identifier(eventsProperty), attrs),
